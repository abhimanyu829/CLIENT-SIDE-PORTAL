...
function LoginForm() {
  ...
  const onSubmit = async (data: LoginFormValues) => {
    ...
    try {
      ...
      if (result.status === "complete") {
        ...
        // Sync with db — fire-and-forget, non-fatal if it fails
        // The Clerk webhook will reconcile if this call fails
  ...
  const handleGithubSignIn = async () => {
    ...
    try {
      ...
      if (err.errors && err.errors.length > 0) {
        setError(err.errors[0].longMessage || "GitHub sign-in failed");
      } else {
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-50 p-4">
      <div className="w-full max-w-md p-8 space-y-6 bg-white rounded-xl shadow-lg border border-gray-100">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Button variant="outline" type="button" onClick={handleGoogleSignIn} className="w-full" disabled={!isLoaded}>
          ...
          <Button variant="outline" type="button" onClick={handleGithubSignIn} className="w-full" disabled={!isLoaded}>
