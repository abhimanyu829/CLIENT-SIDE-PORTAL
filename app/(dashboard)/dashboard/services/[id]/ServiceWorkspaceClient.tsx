...
export default function ServiceWorkspaceClient({ serviceId }: { serviceId: string }) {
  ...
  async function requestAddon(addonId: string) {
    const response = await fetch(`/api/services/${serviceId}/upgrades`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ addonId }) })
    const payload = await response.json()
    if (!response.ok) { setError(payload.error ?? "Unable to request upgrade"); return }
    await load()
  }
