...
export default async function ApisPage() {
  await requireServiceCenterAccess("api-development")

  const [category, services] = await Promise.all([
    db.serviceCategory.findUnique({ where: { slug: "api-development" }, select: { name: true, slug: true } }),
    db.servicePage.findMany({
      where: { category: { slug: "api-development" } },
  ...
  return <ServiceVerticalShell config={SERVICE_CENTER_CONFIG["api-development"]} categorySlug="api-development" categoryLabel={category?.name} services={services as any} />
}
