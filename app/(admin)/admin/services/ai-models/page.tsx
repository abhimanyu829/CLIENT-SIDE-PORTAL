...
export default async function AIModelsPage() {
  await requireServiceCenterAccess("ai-model-development")

  const [category, services] = await Promise.all([
    db.serviceCategory.findUnique({ where: { slug: "ai-model-development" }, select: { name: true, slug: true } }),
    db.servicePage.findMany({
      where: { category: { slug: "ai-model-development" } },
  ...
  return <ServiceVerticalShell config={SERVICE_CENTER_CONFIG["ai-model-development"]} categorySlug="ai-model-development" categoryLabel={category?.name} services={services as any} />
}
