...
export async function POST(req: NextRequest) {
  try {
    ...
    logger.error({ error }, "POST /api/admin/services/emails/test")
    return NextResponse.json({ success: false, error: { message: "Unable to send service test email" } }, { status: 500 })
  }
}
