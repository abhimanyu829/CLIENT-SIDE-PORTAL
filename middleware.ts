...
export default clerkMiddleware(async (clerkAuth, req: NextRequest) => {
  ...
  // ── SSO Callback — MUST be allowed through without any redirect ──
  // Clerk's AuthenticateWithRedirectCallback needs to run fully before
  // the session is established. Redirecting here breaks OAuth flows.
  ...
  // Auth pages: login & register only (sso-callback is already handled above)
  const isAuthPage =
