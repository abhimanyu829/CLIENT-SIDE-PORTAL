...
function ServiceCardView({ service }: { service: ServiceCard }) {
  return (
    <Link
      ...
      onClick={() => track({ eventType: "CLICK", servicePageId: service.id })}
      className="group relative min-w-[260px] max-w-[280px] overflow-hidden rounded-3xl border border-white/80 dark:border-zinc-800/80 bg-gradient-to-br from-slate-50/90 via-indigo-50/40 to-purple-50/60 dark:from-zinc-900/90 dark:via-purple-950/20 dark:to-indigo-950/30 p-5 shadow-md shadow-slate-200/50 dark:shadow-none transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-purple-500/15 flex flex-col justify-between"
    >
      {/* Translucent Liquid Gradient Orbs */}
      <div className="absolute inset-0 rounded-3xl overflow-hidden pointer-events-none z-0">
        <div className="absolute top-0 right-0 w-44 h-44 bg-gradient-to-bl from-purple-300/40 via-pink-200/35 to-blue-200/20 dark:from-purple-900/30 dark:to-blue-950/10 rounded-full blur-xl transform translate-x-10 -translate-y-10" />
        <div className="absolute bottom-0 right-0 w-36 h-36 bg-gradient-to-tl from-indigo-300/35 via-purple-200/30 to-sky-200/25 dark:from-indigo-900/30 dark:to-purple-950/10 rounded-full blur-xl transform translate-x-8 translate-y-8" />
      </div>

      <div className="relative z-10">
        <div className="flex items-start justify-between gap-4">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-amber-500/10 text-amber-700 dark:text-amber-400 backdrop-blur-xs">
            <Sparkles className="h-5 w-5" />
          </div>
          {service.category && (
            <span className="text-xs font-semibold text-muted-foreground bg-background/70 backdrop-blur-xs px-2.5 py-0.5 rounded-full border border-border/50">
              {service.category.name}
            </span>
          )}
        </div>
        <h3 className="mt-4 font-bold text-foreground group-hover:text-amber-700 dark:group-hover:text-amber-400 transition-colors">
          {service.title}
        </h3>
      ...
      <div className="relative z-10 pt-4">
        {service.tags.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mb-4">
            {service.tags.slice(0, 3).map((tag) => (
              <span
                key={tag.slug}
                className="rounded-full bg-background/80 backdrop-blur-xs border border-border/50 px-2 py-0.5 text-[10px] font-semibold text-muted-foreground"
              >
                {tag.name}
              </span>
            ))}
        ...
        <div className="flex items-center text-sm font-bold text-amber-700 dark:text-amber-400">
