...
export default function DashboardSidebar() {
  ...
  const { user: internalUser } = useInternalUser()
  ...
  const displayName =
    ...
    internalUser?.name ||
    "User"
  const displayEmail = clerkUser?.primaryEmailAddress?.emailAddress || internalUser?.email || ""
    ...
    >
      ...
      <div className="flex items-center justify-between px-3 pt-4 pb-3 border-b border-white/5">
        ...
        <button
          ...
          className="ml-auto rounded-lg p-1 text-zinc-600 transition-colors hover:bg-white/5 hover:text-zinc-300"
          ...
          <div key={group.section}>
            ...
            {group.items.map((item) => (
              <Link key={item.href} href={item.href}>
                <div
                  className={`ds-item ${isActive(item.href) ? "ds-item-active" : "text-zinc-500"}`}
                  title={collapsed ? item.label : undefined}
                >
                  <span className={`text-base shrink-0 ${isActive(item.href) ? "text-purple-400" : ""}`}>
                    {item.icon}
                  </span>
                  {!collapsed && (
                    <>
                      <span className="text-sm font-semibold">{item.label}</span>
                      {item.badge !== undefined && item.badge && <span className="ds-badge">{item.badge}</span>}
                    </>
                  )}
                </div>
              </Link>
      ...
      <div className="border-t border-white/5 px-2 pb-3 pt-3">
        <div className={`ds-item ${collapsed ? "justify-center" : ""}`}>
          ...
          {!collapsed && (
            <div className="min-w-0 flex-1">
              <p className="truncate text-xs font-bold text-zinc-300">{displayName}</p>
              <p className="truncate text-[10px] text-zinc-700">{displayEmail}</p>
            </div>
          )}
        </div>
        {!collapsed && (
          <SignOutButton redirectUrl="/login">
            <button className="ds-item mt-1 w-full text-left text-xs text-zinc-700 hover:bg-red-500/5 hover:text-red-400">
              <span>→</span>
              <span>Sign out</span>
            </button>
          </SignOutButton>
